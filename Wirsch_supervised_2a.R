...
setwd("/Users/ryanwirsch/Desktop/SPRING 21/PLSC 497")
...
# some pre-processing (the rest will let dfm do)   STRIP OUT ALL APOSTROPHES
...
By lowering it from .8 to .2 there isnt nearly as many feautures for the performance to run off of, therefor it will be weaker performance. As a result it will be less acurate and not as exact. 
...
The prior arguement refers to prior probabilities assigned to the training classes and allows for replication from other individuals. The default value is uniform priors, whichs allows for the unconditional probability to be the same for one as any other. 
...
Docfreq uses the class priors from the relative proportions of class documents used in a set. This should make my outcome weighted and less smooth and uniform. Based on the plot, I observed an overall shift toward no smooth. 
